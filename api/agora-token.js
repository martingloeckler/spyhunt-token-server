...
/**
 * Verifiziert ein Firebase ID Token über die Firebase REST API.
 * Kein firebase-admin nötig – spart >50MB Bundle-Größe.
 */
async function verifyFirebaseToken(idToken) {
  ...
  if (!response.ok) {
    ...
    throw new Error(err?.error?.message ?? 'Token ungültig');
  }
...
module.exports = async function handler(req, res) {
  ...
  let firebaseApp;
  try {
    firebaseApp = getAdminApp();
  } catch (err) {
    console.error('[agora-token] Firebase Admin Init fehlgeschlagen:', err.message);
    return res.status(500).json({ error: 'Server misconfigured', detail: err.message });
  }
